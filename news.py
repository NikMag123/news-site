...
MODEL = os.getenv("OPENAI_Mimport os
import json
import base64
import requests
import xml.etree.ElementTree as ET
...
4. Текст обязательно должен состоять из 3 фраз в таком порядке:
   - что произошло
   - что это значит
   - кому это может быть важно
5. Пиши просто, по-человечески, но профессионально.
6. Не превращай текст в рекламу.
7. Сохраняй смысл исходника.
